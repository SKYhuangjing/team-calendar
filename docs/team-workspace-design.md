...
## 0.1 后续修订：显式人员借调（2026-06-18）

0.0.4 最初采用“跨团队排期即借调、无需独立关系”的简化设计。该设计只能描述已经发生的跨团队投入，无法表达“已允许借调但尚未排期”的候选资格，导致新增排期必须先看到全体人员，形成循环依赖。

现行规则修订为：

- `people.home_team_id` 仍是唯一正式归属。
- `person_team_loans(person_id, target_team_id, start_date, end_date, note)` 表示有期限的借调资格，不改变正式归属。
- 跨团队新增或调整排期时，排期区间必须被一条借调区间完整覆盖；后端是最终校验边界。
- 排期人员候选 = 项目团队正式成员 + 覆盖当前排期区间的借调人员。
- 既有跨团队排期按原日期区间幂等迁移为借调记录；历史排期仅修改工时/备注时不被新规则阻断。
- 设置页分别展示正式成员和借调人员；把人员移动到其他团队时必须明确选择“修改正式归属”或“借调”。

本文后续出现的“排期天然支持借调/无需特殊机制”均作为 0.0.4 历史设计记录保留，不再代表当前规则。
...
| 跨团队借调 | 显式 `person_team_loans` | 借调先定义参与资格，排期再记录实际投入；`assignments` 仍不冗余 team_id。 |
