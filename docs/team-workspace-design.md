...
### 5.1 团队视图统计口径（待定，见 Q3）

团队视图下「已分配/负载」的分子分母口径需明确，推荐：
...
按「是否需要评审拍板」分两类。落地只需处理 14.1 的两项。

### 14.1 需要确认（影响产品语义，请评审定夺）

| # | 问题 | 选项 / 影响 | 推荐 |
| --- | --- | --- | --- |
| Q1 | 团队视图统计口径 | A. 分子含借调工时、分母仅 home 成员产能 → 负载率偏高（含义「团队项目占用了多少人力」）；B. 仅算 home 成员在本团队工时 → 更准但低估跨团队协作量 | **A**，全局视图补准确负载（见 5.1） |
| Q5 | 团队视图「人员视图」行集合 | A. 参与 X 团队项目的人（含借调，能看到谁在支援本团队）；B. 仅 `home_team=X` 的人（只看本团队成员，借调者不出现在该视图） | **A**（含借调），排期条只渲染本团队项目；负载/冲突仍全局算（见 4.2） |

### 14.2 不需要确认（已有明确默认，按此执行）

| # | 问题 | 默认 | 说明 |
| --- | --- | --- | --- |
| Q2 | `focusDate` 是否 per-team | 全局 | 翻页为临时视图操作，按 team 切分收益小。若后续要求与 viewMode per-team 保持一致，加 `rc_focusDate:<teamId>` 即可 |
| Q3 | 新建团队首次切换的默认偏好来源 | 复制全局档（`settings.team_id=''`）值 | 也可硬编码默认（30d / 8 / 空 printOptions），二选一均合理 |
| Q4 | 默认团队形态 | **已定** | 显式默认团队 `tm_default`（实体化、不可删），强制归属，`''` 不进数据字段（见 9.1 / 11） |
| Q6 | 团队级只读分享（按 team 过滤的 `/api/share`） | 本期不做，预留 | 归 0.1.0 访问控制首个落点 |
